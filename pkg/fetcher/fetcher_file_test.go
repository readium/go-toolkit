...
func TestFileFetcherLengthNotFound(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/unknown"})
...
func TestFileFetcherHrefInMap(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/file_href"})
...
func TestFileFetcherDirectoryFile(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/dir_href/text1.txt"})
	bin, err := resource.Read(0, 0)
	assert.Nil(t, err)
...
func TestFileFetcherSubdirectoryFile(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/dir_href/subdirectory/text2.txt"})
	bin, err := resource.Read(0, 0)
	assert.Nil(t, err)
...
func TestFileFetcherDirectoryNotFound(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/dir_href/subdirectory"})
	_, err := resource.Read(0, 0)
	assert.Equal(t, NotFound(err.Cause), err)
}

func TestFileFetcherDirectoryTraversalNotFound(t *testing.T) {
	resource := testFileFetcher.Get(pub.Link{Href: "/dir_href/../text.txt"})
	_, err := resource.Read(0, 0)
