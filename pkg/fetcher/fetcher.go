...
// A [Fetcher] that a context can be passed to for remote cases.
...
type RemoteFetcher interface {
	 ...
	 * Otherwise, HREFs are sorted alphabetically.
	 */
	LinksWithContext(ctx context.Context) (manifest.LinkList, error)
	 ...
	 * actually fetching it, such as HTTP. Therefore, errors are handled at the Resource level.
	 */
	GetWithContext(ctx context.Context, link manifest.Link) Resource
}
